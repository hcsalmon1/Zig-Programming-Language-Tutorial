...
pub fn processWhile(allocator:*Allocator, convert_data:*ConvertData, node:*ASTNode) ConvertError!void {
    ...
    if (node.children == null) {
        convert_data.error_detail = "node.children is null in if";
        return ConvertError.Node_Is_Null;
    }

    try go_body_script.processBody(allocator, convert_data, node.right.?);
...
pub fn processIfStatement(allocator:*Allocator, convert_data:*ConvertData, node:*ASTNode, add_tabs_and_nl:bool) ConvertError!void {
    //Node_type = ASTNode_Return;
    //Token = return_token; "return"
    //Right = value_node;
    ...
    for (0..child_count) |i| {
        ...
        switch (child.node_type) {
            ASTNodeType.IfStatement => {
                ...
                try processIfStatement(allocator, convert_data, child, false);
            },
            ASTNodeType.Else => {
                convert_data.generated_code.append(allocator, " else {") catch return ConvertError.Out_Of_Memory;
                ...
                try go_body_script.processBody(allocator, convert_data, child.right.?);
