...
const FunctionDefinition = structs_script.FunctionDefinition;
const VariableDefinition = structs_script.VariableDefinition;
...
pub fn printAstError(allocator:*Allocator, ast_data:ASTData, code:[]const u8) !void {
    ...
    //PrintErrorTrace(ast_data.ErrorTrace);
}
...
pub fn printFunctionNodeDetails(function_definition:FunctionDefinition) void {
    //name:[]const u8,
    //return_type_node:*ASTNode,
    //parameters:?ArrayList(*ASTNode),

    print("\t{s}", .{printing_script.CYAN});
    printTypeNodeRecursive(function_definition.return_type_node);
    print("{s}", .{printing_script.RESET});
    print("{s}{s}{s}(", .{
        printing_script.CREAM,
        function_definition.name,
        printing_script.RESET,
    });
    const parameters:?ArrayList(*ASTNode) = function_definition.parameters;
    printParameters(parameters);
    print(")\n", .{});
}

pub fn printVariableNodeDetails(variable_definition:VariableDefinition) void {
    //name:[]const u8,
    //return_type_node:*ASTNode,
    //parameters:?ArrayList(*ASTNode),

    print("\t{s}", .{printing_script.CYAN});
    printTypeNodeRecursive(variable_definition.type_node);
    print("{s}{s}\n", .{
        printing_script.RESET,
