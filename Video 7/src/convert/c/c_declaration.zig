...
pub fn processDeclaration(allocator:*Allocator, convert_data:*ConvertData, node:*ASTNode, add_new_line:bool, add_tabs:bool) ConvertError!void {
    ...
    var c_type:?string = c_utils_script.convertToCType(type_token);

    if (c_type == null) {
        c_type = type_token.Text;
    }
    ...
    const value:string = try print_expression_script.printExpression(allocator, convert_data, node.right.?, false);
...
fn processDeclarationWithFlatten(allocator:*Allocator, convert_data:*ConvertData, node:*ASTNode) ConvertError!void {
    ...
    const c_type:?string = c_utils_script.convertToCType(type_token);
    ...
    const final_value:string = try flatten_expression_script.flattenExpression(allocator, convert_data, node.right.?, true, &statements);
