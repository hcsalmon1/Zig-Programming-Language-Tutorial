...
pub fn getTokenType(input:[]const u8) TokenType {
    ...
    if (twoSlicesAreTheSame(input, constants_script.STRUCT)) return TokenType.Struct;
    ...
    if (twoSlicesAreTheSame(input, constants_script.ENUM)) return TokenType.Enum;
    if (twoSlicesAreTheSame(input, constants_script.PRINTF)) return TokenType.Printf;
