...
fn fillStruct(allocator:*Allocator, ast_data:*ASTData) AstError!*ASTNode {
    ...
    while (ast_data.tokenIndexInBounds()) {
        ...
        if (second_token.Type != TokenType.Identifier) {
            ...
            ast_data.error_detail = "Missing identifier in struct";
        ...
        member_node.left = try ast_node_utils_script.createTypeNode(allocator, first_token);
        member_node.token = second_token;
