...
fn convertCode(arena_allocator:*Allocator, code:string, compiler_settings:*const CompilerSettings) !string {
	...
	//build ASTs
    const ast_nodes:ArrayList(*ASTNode) = ast_script.buildASTs(arena_allocator, token_list, code, compiler_settings) catch |err| {
        return err;
    };
...
fn printOutput(arena_allocator:*Allocator, compiler_settings:*const CompilerSettings) void {
    ...
    const code:string = code_sample_script.error_sample_script.ERROR_4;
